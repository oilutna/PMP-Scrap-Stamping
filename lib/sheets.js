...
const ACTION_HEADERS = ['ID', 'Fecha registro', 'Fecha scrap', 'Departamento', 'Problema', 'Contramedida', 'Responsable', 'Fecha compromiso', 'Estatus', 'Notas', 'Última actualización'];
...
async function ensureActionsTab(sheets) {
  ...
  const header = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${ACTIONS_TAB}!A1:K1` });
  if (!header.data.values?.length) {
    await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: `${ACTIONS_TAB}!A1:K1`, valueInputOption: 'RAW', requestBody: { values: [ACTION_HEADERS] } });
  }
}
...
export async function getCountermeasures() {
  ...
  const response = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${ACTIONS_TAB}!A2:K10000` });
  return (response.data.values || []).filter((row) => row[0]).map((row) => ({
    id: row[0], createdAt: row[1], scrapDate: row[2], department: row[3], problem: row[4], action: row[5], owner: row[6], dueDate: row[7], status: row[8] || 'Abierta', notes: row[9] || '', updatedAt: row[10] || row[1],
  })).reverse();
}
...
export async function addCountermeasure(input) {
  ...
  const item = {
    ...
    dueDate: input.dueDate || '', status: input.status || 'Abierta', notes: input.notes || '', updatedAt: now,
  };
  await sheets.spreadsheets.values.append({
    spreadsheetId: SHEET_ID, range: `${ACTIONS_TAB}!A:K`, valueInputOption: 'USER_ENTERED', insertDataOption: 'INSERT_ROWS',
    requestBody: { values: [[item.id, item.createdAt, item.scrapDate, item.department, item.problem, item.action, item.owner, item.dueDate, item.status, item.notes, item.updatedAt]] },
  });
  return item;
}
...
export async function updateCountermeasure(id, input) {
  ...
  const item = {
    ...
    dueDate: input.dueDate ?? row[7], status: input.status ?? row[8], notes: input.notes ?? row[9], updatedAt: new Date().toISOString(),
  };
  await sheets.spreadsheets.values.update({
    spreadsheetId: SHEET_ID, range: `${ACTIONS_TAB}!A${index + 2}:K${index + 2}`, valueInputOption: 'USER_ENTERED',
    requestBody: { values: [[item.id, item.createdAt, item.scrapDate, item.department, item.problem, item.action, item.owner, item.dueDate, item.status, item.notes, item.updatedAt]] },
  });
  return item;
}
