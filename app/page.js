...
function DailyAnalysis(props) {
  ...
  return <section className="animate-rise">
    ...
    <div className="mb-5 rounded-2xl border border-cyan/25 bg-gradient-to-r from-cyan/[.08] to-violet/[.06] p-4 sm:p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-5"><MeetingMetric label="Costo del día" value={money(meetingCost)} tone="cyan" /><MeetingMetric label="Piezas" value={meetingQty.toLocaleString()} tone="violet" /><MeetingMetric label="Vs. día anterior" value={previousDailyDate ? `${costDelta > 0 ? '+' : ''}${costDelta.toFixed(1)}%` : 'N/A'} tone={costDelta > 0 ? 'coral' : 'teal'} /><MeetingMetric label="Acciones vencidas" value={overdueActions} tone={overdueActions ? 'coral' : 'teal'} /><MeetingMetric label="Issues sin acción" value={issuesWithoutAction} tone={issuesWithoutAction ? 'amber' : 'teal'} /></div>
    </div>
    <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
