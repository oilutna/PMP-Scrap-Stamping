...
const EMPTY_ACTION = { scrapDate: '', week: '', department: '', cause: '', problem: '', action: '', owner: '', dueDate: '', status: 'Abierta', notes: '' };
const STATUS_STYLE = {
...
export default function Home() {
  ...
  const dailyByDept = useMemo(() => groupSum(dailyFiltered, (r) => r.depto_responsible).sort((a, b) => b.cost - a.cost), [dailyFiltered]);
  ...
  const dailyDeptIssues = useMemo(() => dailyByDept.slice(0, 3).map((department) => ({
    ...department,
    issues: groupSum(dailyFiltered.filter((r) => (r.depto_responsible || 'Sin asignar') === department.name), (r) => r.reason)
      .sort((a, b) => b.cost - a.cost).slice(0, 3),
  })), [dailyByDept, dailyFiltered]);
  ...
  function openActionForm(prefill = {}) {
    setForm({ ...EMPTY_ACTION, week: dailyFilters.week === 'all' ? '' : dailyFilters.week, ...prefill });
    setShowForm(true);
  }
    ...
    <main className="mx-auto min-h-screen max-w-[1500px] px-4 py-5 sm:px-6 lg:px-8">
      ...
      {status === 'ready' && <>
        ...
        {tab === 'overview' ? <Overview totalCost={totalCost} totalQty={totalQty} byShift={byShift} byDept={byDept} byArea={byArea} byModel={byModel} byReason={byReason} weeklyTrend={weeklyTrend} /> :
          <DailyAnalysis dailyMode={dailyMode} setDailyMode={setDailyMode} dailyPivot={dailyPivot} byDept={dailyByDept} dailyReasons={dailyReasons} dailyDeptIssues={dailyDeptIssues} actions={dailyActions} actionsStatus={actionsStatus} openActions={openActions} closedActions={closedActions} actionCoverage={actionCoverage} showForm={showForm} openActionForm={openActionForm} closeActionForm={closeActionForm} form={form} setForm={setForm} formError={formError} saving={saving} saveAction={saveAction} saveActionRow={saveActionRow} changeStatus={changeStatus} weeks={options.week} />}
      </>}
    </main>
  );
}
...
function DailyAnalysis(props) {
  const { dailyMode, setDailyMode, dailyPivot, byDept, dailyReasons, dailyDeptIssues, actions, actionsStatus, openActions, closedActions, actionCoverage, showForm, openActionForm, closeActionForm, form, setForm, formError, saving, saveAction, saveActionRow, changeStatus, weeks } = props;
  return <section className="animate-rise">
    <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
      ...
      <button onClick={() => openActionForm()} className="flex items-center justify-center gap-2 rounded-xl bg-cyan px-4 py-2.5 text-sm font-bold text-void shadow-cyan hover:bg-[#6bd8ef]"><Plus size={17} /> Nueva contramedida</button>
    </div>
    <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
...
function DepartmentIssueBoard({ departments, actions, changeStatus, openActionForm }) {
  ...
  return <div className="grid gap-4 xl:grid-cols-3">{departments.map((department, departmentIndex) => <div key={department.name} className="rounded-xl border border-line bg-void/35 p-4"><div className="mb-4 flex items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-cyan/10 font-mono text-xs font-bold text-cyan">0{departmentIndex + 1}</span><div className="min-w-0"><p className="truncate text-sm font-bold text-ink">{department.name}</p><p className="mt-1 text-[11px] text-mute">{money(department.cost)} · {department.qty.toLocaleString()} pzas</p></div></div><div className="space-y-3">{department.issues.map((issue, issueIndex) => { const linked = actions.filter((item) => item.department === department.name && item.cause === issue.name); return <div key={issue.name} className="rounded-lg border border-line/80 bg-panel/65 p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="text-[10px] font-mono uppercase text-coral">Issue 0{issueIndex + 1}</p><p className="mt-1 line-clamp-2 text-xs font-semibold text-ink">{issue.name}</p><p className="mt-1 text-[10px] text-mute">{money(issue.cost)} · {issue.qty.toLocaleString()} pzas</p></div><span className={`shrink-0 rounded-full border px-2 py-1 text-[9px] font-bold ${linked.length ? 'border-teal/30 bg-teal/10 text-teal' : 'border-amber/30 bg-amber/10 text-amber'}`}>{linked.length ? `${linked.length} acción${linked.length > 1 ? 'es' : ''}` : 'Sin acción'}</span></div>{linked.map((item) => <div key={item.id} className="mt-2 rounded-md border border-line/60 bg-void/45 p-2"><div className="flex items-center justify-between gap-2"><p className="truncate text-[10px] text-mute">{item.owner || 'Sin responsable'}</p><select value={item.status} onChange={(e) => changeStatus(item, e.target.value)} className={`rounded-full border px-2 py-1 text-[9px] font-semibold outline-none ${STATUS_STYLE[item.status] || STATUS_STYLE.Abierta}`}><option>Abierta</option><option>En proceso</option><option>Implementada</option><option>Cerrada</option></select></div><p className="mt-1 line-clamp-2 text-[10px] text-ink">{item.action}</p></div>)}<button onClick={() => openActionForm({ department: department.name, cause: issue.name, problem: issue.name })} className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg border border-cyan/25 bg-cyan/5 px-2 py-2 text-[10px] font-bold text-cyan hover:bg-cyan/10"><Plus size={12} /> Agregar contramedida</button></div>; })}</div></div>)}</div>;
}
